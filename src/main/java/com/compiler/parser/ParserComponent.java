...
import static com.compiler.model.TokenType.*;
...
public class ParserComponent {
    ...
    private void generateTable() {
        ListNode k;
        if (tSym.first == null) {
            SemanticLine l = new SemanticLine(newLine);
            k = new ListNode();
            k.line = l;
            k.next = null;
            tSym.first = k;
    ...
    private void search() {
        ...
        if ((currentListNode.line.name.compareTo(newLine.name) != 0) || (currentListNode.line.size.compareTo(newLine.size) != 0)) {
            smbTableError(1);
        }
    }
